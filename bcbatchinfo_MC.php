...
// list any Stock (works orders) associated with batch
...
for ($ki = 0; $ki < $numassigned; ++$ki)
{
    ...
    $item = $assignedrow[1];
    //output information on previously assigned batches
    echo "<tr><td><a href='bcsalesview.php?item=$item&order=$order&Show=0'>$order item $item</td>";
...
// sets up the table for information to do with operation progress within batch    
    ...
    echo"<table class = tablefloatleft border='1'>
         ...
         <th width = 4%>Removed to stock</th>
         <th width = 4%>Moved from stock</th>
         <th width = 4%>Number In Stock </th>
...
for ($f2 = 0; $f2 < $opcount; ++$f2)
    {
        ...
        if ((isset($oporder[$f2]) ? $oporder[$f2] : null) != 0 || $Build_Type == "Full")
            ...
            if ($Build_Type != 'Full')
            {
                ...
                echo "<tr><td><a href='bcbatchinfo.php?op=$row2[0]'>$Global_loop_counter</td>";
            }
            else
            {
				$Global_loop_counter = $Global_loop_counter + 1;
           ...
           // echo $row2[0];
            echo "<td><a href='bcbatchinfo.php?op=$row2[0]'>$operation_number</td>";// global op no
            echo "<td><a href='bcbatchinfo.php?op=$row2[0]'>$row2[1]</td>";
