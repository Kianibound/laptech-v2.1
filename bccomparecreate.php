...
<table class = floatleft border='4' cellpadding='2'
                        ...
                        <th colspan='2' align='center'>Compare time taken between order entry and Batch creation</th>
                        <form method='post' action='bccomparecreate.php??'
...
$result1 = queryMysql($mysqli, "SELECT * FROM bcsalesorders WHERE Date_Entered BETWEEN '$req_date' AND '$to_date'");
...
$result2 = queryMysql($mysqli, "SELECT * FROM bcbatchinfo WHERE create_Date BETWEEN '$req_date' AND '$now'");
//$result = queryMysql($mysqli, "SELECT * FROM bcoperationsdone WHERE DATE(start) = '$req_date' ORDER BY '$Arrange'");
...
echo"   <table class = tablefloatleft border='4'cellpadding='2'
                    ...
                    echo "
                    <th>Date/time order entered</th>
                    <th>date/time batch created</th>
                    <th>batch number</th>
            ...
            for ($ki = 0; $ki < $num1; ++$ki)
            {
                ...
                $match = 0;
                $result3 = queryMysql($mysqli, "SELECT * FROM bcsalestobatch WHERE salesorder = '$row1[0]' AND item = '$row1[1]'");
                ...
                for ($kx = 0; $kx < $num2; ++$kx)
                {
                    ...
                    if($batchno == $row2[0])
                    {
                        ...
                        {
                            $datetime2 = new DateTime($row2[12]);
                            //echo "blah blah blah";
                        }
                    }                     
                ...
                $datetime1 = new DateTime($row1[11]);
            ...
            echo"   <tr bgcolor=$background>
                    <td><a href='bcsalesview.php?order=$row1[0]&item=$row1[1]'> ".$row1[0]."</td>
                    <td>$row1[1]</td>";
                    //<td> ".$row1[1]." ".$OpName."</td>
                ...
                if ($match == 1)
                {
                    ...
                    <td><a href='bcbatchinfo.php?view=$batchno'> $batchno </td>";
                }
                else
                {
            ...
            Sales Total  = $total <br />
            Total missing target for creation of batch = $total_out<br /><br />
