...
if ($localOpNo !=0)
{
    ...
    {//if build is not full
        ...
        for ($i = 0; $i < $opcount+1; ++$i)
        {
            ...
            $lastop = findlastoplocal($mysqli, $localop,$batchno);
            //echo "$lastop <br />";
        }
    }
            ...
            if ($available >= ($Quantity + $Scrapped ))
            {
              echo '<br />number of units available for this operation =   '.$available.'   ';
                if ($fail == "") 
                {
                    ...
                    $status = queryMysql($mysqli, "INSERT INTO bcoperationsdone VALUES('$batchno','$OpNo',
                    ...
                    if ($started == 0)
                    {
                        $status = queryMysql($mysqli, "UPDATE bcbatchinfo SET started = '$now', status = '1'
