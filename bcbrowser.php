...
if (isset($_POST['batchno']))
	{
		...
		setcookie("batchno", $batchno, time()+3600);
	}
        
if (isset($_POST['previousPage']))
	{
		$previousPage = ($_POST['previousPage']);
		setcookie("previousPage", $previousPage, time()+3600);
	}
else
{
    $previousPage = "bcmessages.php";
}
...
if (isset($_POST['productcode']))
	{
		...
		setcookie("productcode", $productcode, time()+3600);
	}
...
if (isset($_POST['qtystart']))
	{
		...
		setcookie("qtystart", $qtystart, time()+3600);
	}

if (isset($_POST['qtyreq']))
	{
		$qtyreq = ($_POST['qtyreq']);
		setcookie("qtyreq", $qtyreq, time()+3600);
	}

if (isset($_POST['OpNo']))
	{
		$OpNo = ($_POST['OpNo']);
		setcookie("OpNo", $OpNo, time()+3600);
	}

if (isset($_POST['operation']))
	{
		$operation = ($_POST['operation']);
		setcookie("operation", $operation, time()+3600);
	}

if (isset($_POST['relateddocs']))
	{
		$relateddocs = ($_POST['relateddocs']);
		setcookie("relateddocs", $relateddocs, time()+3600);
	}


// brings in our headers 
...
?>
<tr class="row two">
	<td></td><td class="icon"><img alt="dir" src="?img=directory" /></td>
...
if (isset($_POST['relateddocs']))
	...
	foreach ($this->files as $file)
	{
		...
		print"<td><form method='post' name='link' action='$previousPage'>";	
		print"<input type='hidden' maxlength='255' name='linkin' value='".$this->location->getDir(false, true, false, 0).$file->getNameEncoded()."'>";
		print"<input type='hidden' maxlength='32' name='OpNo' value='$OpNo' />";
		print"<input type='hidden' maxlength='32' name='operation' value='$operation' />";
		print"<input type='hidden' maxlength='64' name='relateddocs' value='$relateddocs' />";
		print"<input type='hidden' maxlength='32' name='batchno' value='$batchno' />";
		print"<input type='hidden' maxlength='32' name='productcode' value='$productcode' />";
		print"<input type='hidden' maxlength='32' name='description' value='$description' />";
		print"<input type='hidden' maxlength='12' name='qtystart' value='$qtystart' />";
		print"<input type='hidden' maxlength='12' name='qtyreq' value='$qtyreq' />";
		print"<input type='submit' name='link' value='link this file'></td></form>";
